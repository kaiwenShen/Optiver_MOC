...
import numpy as np
import utils
target_on_train = np.load('target_on_train.npy')
target_on_test = np.load('target_on_test.npy')
target_all = np.load('target_all.npy')
class Factor_Backtest:
    ...
    def check_in_sample_corr(self):
        ...
        for factor_name, factor_value in self.testing_factors.items():
            for factor_name_, factor_value_ in self.existed_factors.items():
                corr_coef = np.corrcoef(factor_value, factor_value_)
                if corr_coef < 0.4:
                    # pass directly
                    self.passed_factors[f"{factor_name}_train"].append(1)
    ...
    def run_testing(self):
        ...
        for factor_name, factor_value in self.testing_factors.items():
            if sum(self.passed_factors[factor_name]) == 3:

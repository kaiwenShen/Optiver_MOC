...
def add_factor_to_existed_train() -> dict:
    '''
    This function will add the passed factors to the existed factor dictionary.
    ...
    existed_factors: A dictionary that will store all the passed factors.
    '''
    existed_factors = load_json_factors('./in_house_validation/existed_factors.json')
    file_pattern = "./in_house_validation/tmp*.json"
    matching_files = glob.glob(file_pattern)
    print(f"matching files: {matching_files}")
    if len(matching_files) == 0:
        print('No temporary files found, no new factors added, returning existed_factors')
        return existed_factors
    for file in matching_files:
        with open(file, 'r') as json_file:
            tmp = json.load(json_file)
            for key in tmp.keys():
                if key in existed_factors.keys():
                    print(f"Factor {key} already exists in existed_factors, overwriting the value")
                else:
                    print(f"Factor {key} is a new factor, adding the value")
                existed_factors[key] = np.array(tmp[key], dtype=np.float64)
            print(f"{file} added to existed_factors")
    print('storing the updated existed_factors')
    dump_json_factors(existed_factors, './in_house_validation/existed_factors.json')# store the updated existed_factors
    print('existed_factors updated, all temporary files deleted')
    for file in matching_files:
        os.remove(file)
    return existed_factors

def add_factor_to_existed_test() -> dict:
    '''
    This function will add the passed factors to the existed factor dictionary.
    ...
    existed_factors: A dictionary that will store all the passed factors.
    '''
    existed_factors = load_json_factors('./in_house_validation/existed_factor_test.json')
    file_pattern = "./in_house_validation/test_tmp*.json"
    matching_files = glob.glob(file_pattern)
    print(f"matching files: {matching_files}")
    if len(matching_files) == 0:
        print('No temporary files found, no new factors added, returning existed_factors')
    ...
    for file in matching_files:
        with open(file, 'r') as json_file:
            ...
            print(f"{file} added to existed_factors")
    print('storing the updated existed_factors')
    dump_json_factors(existed_factors, './in_house_validation/existed_factor_test.json')# store the updated existed_factors
    print('existed_factors updated, all temporary files deleted')
    for file in matching_files:
        os.remove(file)
    return existed_factors